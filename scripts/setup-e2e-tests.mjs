...
async function hasTablesInDatabase() {
  try {
    const { stdout } = await execAsync(
      'PGPASSWORD=spec psql -h localhost -p 5439 -U spec -d spec_e2e -t -c "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = \'kb\';"'
    );
...
async function startE2EDatabase() {
  ...
  if (await isPortInUse(5439)) {
    logSuccess('E2E database already running on port 5439');
...
async function runMigrations() {
  ...
  const migrationResult = await execAsync(
    'cd apps/server && POSTGRES_HOST=localhost POSTGRES_PORT=5439 POSTGRES_DB=spec_e2e POSTGRES_USER=spec POSTGRES_PASSWORD=spec NODE_ENV=test npx typeorm migration:run -d dist/typeorm.config.js',
    { cwd: PROJECT_ROOT }
  );
