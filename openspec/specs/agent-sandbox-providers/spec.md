# agent-sandbox-providers Specification

## Purpose
Defines how agent sandbox providers are reported and selected: the providers endpoint reports every known provider (gvisor, firecracker, e2b) with live availability and a reason, provider health is refreshed for the whole process lifetime, selection errors enumerate each candidate's rejection reason, and the Sandbox settings UI derives provider choices from reported availability.
