...
### Requirement: Basic Blueprints Command

The CLI SHALL provide a `blueprints` subcommand that reads a config directory and applies its resources to a target Emergent project.

#### Scenario: Apply from local folder

- **WHEN** a user runs `memory blueprints ./my-config`
...
- **WHEN** a user runs `memory blueprints https://github.com/org/repo`
- **THEN** the CLI SHALL fetch the repository contents via the GitHub raw archive endpoint
- **AND** apply the same parsing and apply logic as a local folder
- **AND** support branch/tag refs via URL fragment: `https://github.com/org/repo#main`

#### Scenario: Apply from GitHub URL — private repo

- **WHEN** a user runs `memory blueprints https://github.com/org/private-repo`
- **AND** `MEMORY_GITHUB_TOKEN` is set or `--token <token>` is passed
- **THEN** the CLI SHALL include the token in GitHub archive requests
- **AND** if no token is provided and the repo is private, the CLI SHALL fail with a clear message: `Repository requires authentication. Set MEMORY_GITHUB_TOKEN or pass --token.`

#### Scenario: Empty or missing subdirectories

- **WHEN** the blueprint source has no `packs/` directory
...
By default, the blueprints command SHALL never overwrite existing resources.
...
- **WHEN** a user runs `memory blueprints ./my-config --upgrade`
...
- **WHEN** a user runs `memory blueprints ./my-config --dry-run`
...
- **WHEN** a user runs `memory blueprints ./my-config --dry-run --upgrade`
...
### Requirement: Blueprints Summary

After all files are processed, the CLI SHALL print a summary of actions taken (or planned, for dry run).

#### Scenario: Live blueprints summary

- **WHEN** a blueprints run completes
- **THEN** the CLI SHALL print a summary such as:
  ```
  Blueprints complete: 2 created, 1 updated, 1 skipped, 0 errors
  ```
...
#### Scenario: Blueprints run with errors

- **WHEN** one or more files fail to parse or fail the API call
- **THEN** the CLI SHALL continue processing remaining files
- **AND** include the error count in the summary: `Blueprints complete: 2 created, 0 updated, 0 skipped, 1 error`
- **AND** exit with a non-zero exit code if any errors occurred

### Requirement: Target Project Selection

The blueprints command SHALL apply resources to a specific Emergent project.
