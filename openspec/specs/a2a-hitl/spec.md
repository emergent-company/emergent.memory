# a2a-hitl Specification

## Purpose

Define human-in-the-loop behaviour for A2A tasks in Memory: pausing a task at `TASK_STATE_INPUT_REQUIRED` with the prompt carried in `status.message`, distinguishing an `ask_user` question pause from a tool-approval pause, and resuming by sending a follow-up message that carries the same `taskId` (there is no separate resume route). Includes the resume guards and the documented streaming behaviour across an interruption.
