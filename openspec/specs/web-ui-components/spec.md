...
Defines how the Memory gateway reuses go-daisy's shared UI components instead of carrying local
copies, so dialogs, confirm dialogs, toasts, list sections, the command palette, detail-page
headers, sidebar nav rows, avatars, type accents, and bare card lists stay single-sourced in the
library and the gateway migrates call sites without changing rendered output.
