...
How the web chat gateway and its shared client engine transport and render
streamed assistant text: the gateway forwards upstream token deltas raw (no
per-token markdown render) and emits one authoritative, server-rendered and
sanitized markdown snapshot per turn, while the client appends deltas as plain
text and swaps to the snapshot when it arrives — keeping streaming cost linear
in answer length without adding a second HTML-sanitization boundary.
