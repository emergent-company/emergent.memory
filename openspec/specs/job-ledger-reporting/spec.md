# job-ledger-reporting Specification

## Purpose
Defines how stale-sweep outcomes are reported separately from genuine job failures, with a single source of truth for the stale marker, an additive backwards-compatible response shape, and retention that reclaims terminal rows across every swept table.
