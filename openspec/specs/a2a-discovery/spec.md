# a2a-discovery Specification

## Purpose

Define the A2A Protocol v1.0 discovery surface for Memory: the unauthenticated, tenant-blind global AgentCard served at `/.well-known/agent-card.json`, and the authenticated `GET /extendedAgentCard` that enumerates a project's externally-visible agent definitions as `AgentSkill`s. This capability governs what discovery advertises, how skills are derived from agent definitions, and how authentication is declared — with the hard invariant that public discovery reveals no tenant data.
