...
The CLI SHALL authenticate users via OAuth 2.0 Device Authorization Grant flow and manage credentials securely.

#### Scenario: Initial credential setup

- **WHEN** user runs `memory config set-credentials --email user@example.com`
...
- **THEN** CLI displays "Authentication failed. Run 'memory config set-credentials' to update."
- **AND** exits with non-zero status code

#### Scenario: Unauthenticated status hints at register

- **WHEN** user runs `memory status`
- **AND** no credentials file exists and no API key is configured
- **THEN** CLI SHALL print "Not authenticated."
- **AND** CLI SHALL print a message suggesting both `memory login` (returning users) and `memory register` (new users) as next steps
...
- **WHEN** user runs `memory config set-defaults --org org_123 --project proj_456`
...
- **WHEN** user runs `memory documents list`
...
- **WHEN** user runs `memory documents list --output json`
...
- **WHEN** user runs `memory documents create --file /path/to/doc.pdf`
...
- **WHEN** user runs `memory documents delete doc-123`
...
- **WHEN** user runs `memory documents delete doc-123 --force`
...
- **THEN** displays "Error: Authentication failed. Run 'memory config set-credentials' to update."
...
- **WHEN** user runs `memory chat send "What is in the knowledge base?"`
...
- **WHEN** user runs `memory chat history`
...
- **WHEN** user runs `memory extraction run doc-123`
...
- **WHEN** user runs `memory extraction status job-456`
...
- **WHEN** user runs `memory extraction list-jobs`
...
- **WHEN** user runs `memory server health`
...
- **WHEN** user runs `memory server info`
...
- **WHEN** user runs `memory completion bash`
- **THEN** CLI outputs bash completion script
- **AND** includes instructions for installation

#### Scenario: Generate zsh completion

- **WHEN** user runs `memory completion zsh`
- **THEN** CLI outputs zsh completion script
- **AND** includes instructions for installation

#### Scenario: Command completion

- **WHEN** user types `memory doc<TAB>` in configured shell
- **THEN** shell completes to `memory documents`

#### Scenario: Flag completion

- **WHEN** user types `memory documents list --out<TAB>`
...
- **WHEN** user runs `memory template-packs list`
...
- **WHEN** user runs `memory template-packs get <pack-id>`
...
- **WHEN** user runs `memory template-packs create --file pack.json`
...
- **WHEN** user runs `memory template-packs installed`
...
- **WHEN** user runs `memory template-packs install <pack-id>`
...
- **WHEN** user runs `memory template-packs uninstall <pack-id>`
...
- **WHEN** user runs `memory template-packs compiled-types`
...
- **WHEN** user runs `memory template-packs validate --file pack.json`
...
- **WHEN** user runs `memory serve --docs-port 8080`
...
- **WHEN** user runs `memory serve --mcp-stdio`
...
- **WHEN** user runs `memory serve --mcp-port 3100`
...
- **GIVEN** Claude Desktop config includes memory as MCP server
- **WHEN** Claude Desktop starts
- **THEN** it spawns `memory serve --mcp-stdio`
- **AND** Claude can invoke tools like `documents_list`, `extraction_run`

#### Scenario: Combined serve mode

- **WHEN** user runs `memory serve --docs-port 8080 --mcp-port 3100`
...
The CLI SHALL expose `memory register` as a top-level command, parallel to `memory login`, as the recommended entry point for new users creating an account for the first time.

#### Scenario: Register command is discoverable

- **WHEN** user runs `memory --help`
...
- **WHEN** user runs `memory install-skills --help`
