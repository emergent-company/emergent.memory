...
- **WHEN** a session has stored messages but no run timeline (e.g. a document-classification conversation)
- **THEN** the API returns the session's messages as timeline records rather than an empty timeline
