...
- [x] 1.3 `window.openDialogByID(id)` global (exposed by `webui/static/js/app.js`, delegating to `MemoryApp.openDialog`) + `DialogAutoOpen()` (returns the `data-dialog-autoopen` attribute) in `components/dialog.templ`. Unit test: the attribute is exactly `data-dialog-autoopen=true`, and the global delegates to `MemoryApp.openDialog`. Verify: `go test ./components/...` green.
