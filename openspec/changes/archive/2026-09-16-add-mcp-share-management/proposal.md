...
The Memory backend can expose a project over MCP (read-only share endpoint plus scope-filtered tool catalog), but the web app offers no way to create or manage those shares. Users cannot see existing shares, choose which memory tools an outside agent may call, or copy connection details — they must call the API by hand. This blocks the intended "hand an outside agent a scoped MCP instance" workflow.
...
- The create/edit form lets the admin **select which memory tools** an instance may expose (grouped by category, with a search/filter); the instance's token is scoped automatically from the selected tools.
...
- List existing shares with name, tool count, created date, last used, and status; support rename/update and revoke with confirmation.
...
- Add an entry point from the existing MCP Servers page to the MCP Sharing area.
...
- `mcp-share-management`: gateway API + web UI to create, list, edit, rotate, and revoke project MCP share instances, including the memory-tool picker and the one-time key/snippet reveal.
...
- **Depends on** the Memory backend change `add-mcp-share-instances` in `emergent.memory` (share CRUD + tool catalog + per-instance tool allowlist). Until it lands, the UI is inert; ship both together.
