...
- [x] 1.1 Add `--filter` (repeatable `[]string`) and `--filter-op` (string, default `"eq"`) flags to `graphObjectsListCmd` in `apps/cli/internal/cmd/graph.go`
...
- [x] 2.2 Verify `go test ./apps/cli/...` passes

## 3. Help Text & Docs

- [x] 3.1 Update `graphObjectsListCmd.Long` description to mention `--filter` and `--filter-op` with examples
- [x] 3.2 Update `apps/cli/internal/skillsfs/skills/memory-cli-reference/SKILL.md` — add `--filter` and `--filter-op` entries to the `memory graph objects list` flags table
