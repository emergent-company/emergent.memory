...
- Add child spans for parallel search sub-operations (graph search, text search, relationship search) with proper wait-group synchronization to preserve partial results
...
- Fix async goroutines that detach from parent span context: background message persistence, access timestamp updates in search
- Add child spans for synchronous hot-path operations: RAG search in chat handler, `AddMessage`, `HybridSearch`, `graphService.Create`, `graphService.CreateRelationship`
- Add child spans for document chunking, storage I/O, and quality checker evaluations in extraction
...
- `apps/server-go/domain/chat/handler.go` — async goroutine fixes for message persistence, synchronous RAG search span
...
- `apps/server-go/domain/search/service.go` — wait-group fix for parallel traces, sub-spans for graph/text/relationship search, embedding span, fusion span
- `apps/server-go/pkg/tracing/tracer.go` — adding `StartLinked` and `RecordErrorWithType` helpers
