...
The system SHALL create a child span for each parallel search sub-operation: `search.graph_search`, `search.text_search`, and `search.relationship_search`. Each goroutine SHALL receive the parent span context before being launched, without introducing a shared cancellation context that would fail all searches if one sub-search fails. Each sub-span SHALL record the number of results returned as `emergent.search.sub_result_count` (int) upon completion.
...
The system SHALL ensure that goroutines launched for background work (message persistence, access timestamp updates) carry the parent span context. Background goroutines that outlive the HTTP request lifecycle SHALL use linked spans (not child spans) to preserve trace continuity without depending on a cancelled context. The parent `SpanContext` MUST be extracted before launching the goroutine.
...
### Requirement: Synchronous operations in the chat handler are traced
The system SHALL create a synchronous child span `chat.rag_search` for the knowledge graph retrieval phase before streaming begins.

#### Scenario: RAG search produces a span
- **WHEN** the chat handler performs a search against the graph
- **THEN** a `chat.rag_search` span is created wrapping the synchronous operation

---

### Requirement: Extraction sub-operations are traced
The system SHALL create child spans for distinct phases of the extraction workers, including chunking, document downloading, and quality checking.

#### Scenario: Document downloading is traced
- **WHEN** the document parsing worker downloads a document
- **THEN** a child span `extraction.document_download` is created to record the I/O latency

#### Scenario: Chunking is traced
- **WHEN** the chunk embedding worker processes text chunks
- **THEN** an `extraction.chunking` span wraps the chunk service operation

#### Scenario: Quality checker sub-operations are traced
- **WHEN** the quality checker evaluates extraction results
- **THEN** child spans for `extraction.quality_check` (and any state reads or orphan calculations) appear in the trace waterfall
