## 1. File Format Types (`internal/blueprints/types.go`)

- [x] 1.1 Define `PackFile` struct with `json:` and `yaml:` tags matching the blueprint-format spec (name, version, description, author, license, repositoryUrl, documentationUrl, objectTypes, relationshipTypes, uiConfigs, extractionPrompts)
- [x] 1.2 Define `AgentFile` struct with `json:` and `yaml:` tags matching the blueprint-format spec (name, description, systemPrompt, model, tools, flowType, isDefault, maxSteps, defaultTimeout, visibility, config, workspaceConfig)
- [x] 1.3 Define `ObjectTypeDef` and `RelationshipTypeDef` structs (nested in PackFile)
- [x] 1.4 Define `BlueprintsResult` struct to track per-resource outcome (resource type, name, action: created/updated/skipped/error, error message)

## 2. File Loader (`internal/blueprints/loader.go`)
...
## 3. GitHub Fetcher (`internal/blueprints/github.go`)
...
- [x] 3.3 Extract the tar.gz to a temp directory (`memory-blueprints-*`), stripping the top-level repo prefix so `packs/` and `agents/` are at the root
- [x] 3.4 Return cleanup function (`os.RemoveAll`) for the caller to defer
- [x] 3.5 On HTTP 404 with no token, return error: `repository not found or requires authentication — set MEMORY_GITHUB_TOKEN or pass --token`
...
## 5. Blueprinter (`internal/blueprints/applier.go`)

- [x] 5.1 Implement `Blueprinter` struct holding SDK clients (templatepacks, agentdefinitions), flags (dryRun, upgrade), and output writer
...
- [x] 5.4 For each parsed agent file: check name against existing agents map
  ...
  - found + no `--upgrade` → skip with hint message
- [x] 5.5 In `--dry-run` mode: print `[dry-run] would <action> <type> "<name>"` for every file; make zero API calls
- [x] 5.6 Collect all `BlueprintsResult` entries; print per-resource output line after each action
- [x] 5.7 After processing all files, print summary line: `Blueprints complete: N created, N updated, N skipped, N errors` (or `Dry run complete: ...` variant)

## 6. CLI Command (`internal/cmd/blueprints.go`)

- [x] 6.1 Create `blueprintsCmd` cobra command with `Use: "blueprints <source>"`, `Args: cobra.ExactArgs(1)`
- [x] 6.2 Add flags: `--project` (string, overrides `EMERGENT_PROJECT_ID`), `--upgrade` (bool), `--dry-run` (bool), `--token` (string, GitHub token)
- [x] 6.3 Resolve source: if GitHub URL call `github.FetchGitHubRepo` with `--token` / `MEMORY_GITHUB_TOKEN`, defer cleanup; otherwise treat as local path
- [x] 6.4 Call `loader.LoadDir(dir)` to parse files
- [x] 6.5 Build `Blueprinter` with resolved SDK clients and flags; call `blueprinter.Run()`
- [x] 6.6 Exit non-zero if any `BlueprintsResult` has action `error`
- [x] 6.7 Wire `blueprintsCmd` into `rootCmd` in `init()`
...
- [x] 7.2 Unit test `Blueprinter` with mock SDK clients: create-only (default), skip existing (no `--upgrade`), update existing (`--upgrade`), dry-run outputs correct lines and makes no API calls
