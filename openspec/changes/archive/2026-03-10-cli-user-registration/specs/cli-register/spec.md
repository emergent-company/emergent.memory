...
The CLI SHALL provide an `emergent register` command that guides a new user through account creation using the OAuth 2.0 Device Authorization Grant flow against Zitadel, saves credentials locally, and confirms server-side account provisioning.

#### Scenario: Successful registration flow

- **WHEN** a user runs `emergent register`
...
- **WHEN** `emergent register` completes successfully
...
- **AND** the CLI SHALL suggest running `emergent login` or using the API key method instead
...
- **AND** the CLI SHALL suggest running `emergent status` to check authentication state

### Requirement: CLI E2E test for account confirmation

The test suite SHALL include an E2E test that verifies `GET /api/auth/me` returns a valid user profile when called with a known test token, exercising the account-confirmation step used by `emergent register`.
