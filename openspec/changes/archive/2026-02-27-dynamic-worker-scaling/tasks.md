...
- [x] 9.1 Implement `pkg/syshealth/metrics.go` for Prometheus metrics registration
- [x] 9.2 Add `system_health_score` gauge with zone label
- [x] 9.3 Add `system_io_wait_percent` gauge
- [x] 9.4 Add `system_cpu_load_avg` gauge with period label (1m, 5m, 15m)
- [x] 9.5 Add `system_memory_utilization_percent` gauge
- [x] 9.6 Add `system_db_pool_utilization_percent` gauge
- [x] 9.7 Add `extraction_worker_current_concurrency` gauge with worker_type label
- [x] 9.8 Add `extraction_worker_concurrency_adjustments_total` counter with worker_type, direction, reason labels
- [x] 9.9 Add `extraction_jobs_throttled_total` counter with worker_type label
- [x] 9.10 Update Monitor to publish metrics on each collection cycle
- [x] 9.11 Update ConcurrencyScaler to publish metrics on concurrency changes

## 10. Unit Tests - Health Monitor

- [x] 10.1 Create `pkg/syshealth/monitor_test.go`
- [x] 10.2 Test metric collection with mocked gopsutil functions
- [x] 10.3 Test health score calculation with various metric combinations
- [x] 10.4 Test health zone determination (critical, warning, safe)
- [x] 10.5 Test weighted scoring formula (40% I/O, 30% CPU, 20% DB, 10% Mem)
- [x] 10.6 Test graceful degradation on metric collection failures
- [x] 10.7 Test staleness tracking (metrics older than 2 minutes)
- [x] 10.8 Test Start/Stop lifecycle
- [x] 10.9 Test concurrent GetHealth calls (RWMutex correctness)

## 11. Unit Tests - Concurrency Scaler

- [x] 11.1 Create `pkg/syshealth/scaler_test.go`
- [x] 11.2 Test GetConcurrency with adaptive scaling disabled (returns static value)
- [x] 11.3 Test GetConcurrency in critical health zone (returns min concurrency)
- [x] 11.4 Test GetConcurrency in warning health zone (returns 50% of max)
- [x] 11.5 Test GetConcurrency in safe health zone (returns max concurrency)
- [x] 11.6 Test gradual scaling (max 50% increase per cycle)
- [x] 11.7 Test cooldown period enforcement (5 min for increase, 1 min for decrease)
- [x] 11.8 Test cooldown bypass in critical health zone
- [x] 11.9 Test concurrency bounds enforcement (min, max)
- [x] 11.10 Test stale health data handling (defaults to 50% of max)

## 12. Unit Tests - Worker Integration

- [x] 12.1 Update `graph_embedding_worker_test.go` to test adaptive scaling integration
- [x] 12.2 Test GraphEmbeddingWorker with mock scaler (verify GetConcurrency is called)
- [x] 12.3 Test ChunkEmbeddingWorker with mock scaler
- [x] 12.4 Test DocumentParsingWorker with mock scaler
- [x] 12.5 Test ObjectExtractionWorker with mock scaler
- [x] 12.6 Test worker behavior when scaler returns different concurrency values

## 13. Integration Tests

- [x] 13.1 Create `pkg/syshealth/integration_test.go`
- [x] 13.2 Test end-to-end: Monitor → Scaler → Worker concurrency adjustment
- [x] 13.3 Test simulated health degradation (inject high I/O wait, verify concurrency reduces)
- [x] 13.4 Test simulated health recovery (inject safe metrics, verify gradual concurrency increase)
- [x] 13.5 Test multiple workers with shared Monitor (verify all respond to health changes)
- [x] 13.6 Test configuration API updates (POST new values, verify workers pick up changes)
- [x] 13.7 Test Prometheus metrics are published correctly

## 14. Documentation

- [x] 14.1 Add godoc comments to all exported types and functions in `pkg/syshealth/`
- [x] 14.2 Create `apps/server-go/pkg/syshealth/README.md` with usage examples
- [x] 14.3 Document health score calculation formula and thresholds
- [x] 14.4 Document configuration API changes in API documentation
- [x] 14.5 Add example cURL commands for enabling/disabling adaptive scaling
- [ ] 14.6 Document Prometheus metrics and their labels

## 15. Deployment Preparation

- [x] 15.1 Verify hot reload works (code changes picked up without restart)
- [x] 15.2 Run `nx run server-go:test` to ensure all tests pass
- [x] 15.3 Run `nx run server-go:lint` to ensure code style compliance
- [x] 15.4 Verify no breaking changes to existing worker configuration
- [x] 15.5 Verify default behavior unchanged (adaptive scaling disabled by default)
- [x] 15.6 Test configuration API backward compatibility (legacy fields still work)
- [x] 15.7 Create deployment checklist for gradual rollout
