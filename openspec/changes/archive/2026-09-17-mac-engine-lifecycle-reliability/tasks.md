...
- [x] 6.2 `xcodebuild … test` runs the unit-test bundle green, including the new policy, gate, and port-conflict tests — 358 tests, 0 failures (up from 343; +15 new: 5 policy, 6 gate, 4 port-conflict). Refinements landed in #577 on top of this change are covered by the same tasks and tests: the port probe tests a stopped-but-not-yet-cleared child (`process?.isRunning != true`) rather than only `process == nil`, the start transition is logged only after `proc.run()` succeeds, and the stderr bind-failure scan is line-framed across chunk boundaries
- [x] 6.3 Manual repro on the built app: sign in, switch between the dev and prod accounts, and connect a project — confirm the log shows one `engine start` per genuine change with no stop-storm, no `wrongProject` start, and no engine spawned into a held port. Verified against a build of merged `main` (358 tests green, installed and relaunched on the Mac build machine): exactly 1 `engine start` and 1 stop line since the relaunch, no port-conflict abort, no bind-failure marker, no `.wrongProject` start. The new `[lifecycle]` lines also settled the open question from the investigation — every `applyScope` was preceded by a user-driven `activeAccountDidChange` (`source=signIn` / `switchTo` / `signOut`), so the observed churn was user-driven account switching, not an autonomous reconcile loop. Follow-up found by this repro: hosted unit-test runs also write `[lifecycle]` lines into the real app log (test stub accounts appear in it), which should be guarded like the legacy migrator's hosted-test check.
