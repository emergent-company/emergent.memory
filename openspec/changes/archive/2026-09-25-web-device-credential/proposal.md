...
- **Gateway device branch.** `requireSessionOrKey` falls back to `requireDeviceCredential` when no session is present: it introspects the `emt_*` credential (via memory's `GET /api/auth/me`) for the `device:api` marker, enforces a gateway surface allowlist (`/api/token`, agent picker, chat/session relay, `/api/memories*`), proxies the credential verbatim, and derives project/org from the token — never from raw headers. The credential is accepted on `Authorization: Bearer emt_…` or `X-API-Key` (the iOS client's convention — the same token, validated identically, so this is not a widening).
...
2. **Read-only ceiling confirmed sufficient.** The stored ceiling `device:api + agents:read + data:read` covers the verified device surface — voice room JWT, agent picker, chat/session relay, read-memory browsing — with no known device write feature. No widening was made. (`agents:read` expands to `chat:use`/`skills:read`; `data:read` expands via `ScopeImplies` to the read family **including `schema:read`**, so the effective set is read-only, excluding only `projects:read` and every write/admin/org scope.)
3. **Device credential header — accept `X-API-Key` as well as Bearer.** The iOS `VoiceAgent` presents the credential as `X-API-Key` (5 call sites). Rather than ship an unbuildable-on-this-host Swift change, the gateway accepts the same `emt_*` device credential on `X-API-Key` and `Authorization: Bearer`, validated identically (marker introspection + surface allowlist). This is not a widening — it is the same token through either channel, as the #818 design contemplated. A future iOS migration to `Authorization: Bearer` is worth a follow-up, not this PR.
