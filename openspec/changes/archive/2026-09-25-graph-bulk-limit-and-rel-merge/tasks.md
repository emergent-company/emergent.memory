...
- [x] 2.4 (PR review) Gate the relationship probe by type: `FindSimilarRelationshipInBranch` takes a `relType` param and adds `AND type = ?`; the `service.go` call site passes `src.Type`. Update `TestFindSimilarRelationshipInBranch_TieDeterminism` → `_TypeGate` (same-endpoint/different-type rows now coexist via the unique index and are selected by type). Verify: `go test ./domain/graph/...` passes.
- [x] 2.5 (PR review) Preserve absorbed relationship metadata: the `newRelVersion` literal in the `service.go` absorb branch sets `Label` and `Weight` from `similarRel`. Strengthen `TestMergePolicy_RelationshipSimilarity_AbsorbsDuplicate` to seed a non-nil label/weight and assert they survive; add `TestMergePolicy_RelationshipSimilarity_TypeGate` proving a different-type staged relationship is ADDED (second live HEAD row) with the original relationship unchanged. Verify: `go test ./tests/integration/ -run TestMergePolicySuite` passes.
