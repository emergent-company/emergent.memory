...
- **Regression protection for the leak class.** Graceful shutdown keeps removing its own pool, and any residue from an ungraceful exit is reclaimed once the dead owner's liveness lease goes stale (with the grace period still applied) — possibly not on the very next pass — so steady-state container count is bounded by the configured pool target regardless of restart pattern (design D4).
- **The pool pre-boots only a reconcilable provider.** Warm containers are created through the label-enumerating, lease-capable provider (design D9); a host whose selected provider cannot be enumerated or kept alive by a lease refuses the warm container instead of pre-booting an unreclaimable one.
