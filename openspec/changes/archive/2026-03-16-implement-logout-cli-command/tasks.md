...
- [x] 1.1 Add `RevocationEndpoint` field to `OIDCConfig` struct in `apps/cli/internal/auth/discovery.go`
- [x] 1.2 Create `Revoke(issuerURL, clientID, token, tokenTypeHint string) error` function in a new file `apps/cli/internal/auth/revoke.go` — discovers OIDC config, POSTs to revocation endpoint with `token`, `token_type_hint`, and `client_id` form params, uses 10-second HTTP timeout
- [x] 1.3 Write unit tests for `Revoke` in `apps/cli/internal/auth/revoke_test.go` — test successful revocation, missing revocation endpoint (skip gracefully), network error, timeout, non-2xx response

## 2. Enhanced Logout Command

- [x] 2.1 Add `--all` flag to `logoutCmd` in `apps/cli/internal/cmd/auth.go` (bound to a package-level `logoutAll` bool variable)
...
- [x] 3.1 Update existing `TestLogout` in `apps/cli/internal/cmd/auth_test.go` to verify revocation is attempted (mock HTTP server for OIDC discovery + revocation endpoint)
