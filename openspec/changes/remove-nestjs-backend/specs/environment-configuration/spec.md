...
**`apps/server-go/.env.local`** - User-specific server overrides (gitignored):
...
**And** all server variables SHALL be in `apps/server-go/.env`  
...
2. `apps/server-go/.env` (app-specific config)
