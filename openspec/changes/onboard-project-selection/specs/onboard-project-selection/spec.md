...
- **WHEN** the skill runs `emergent projects list` and one or more projects are returned
- **THEN** the skill presents the list to the user and asks which project to use for onboarding

#### Scenario: No projects exist
- **WHEN** the skill runs `emergent projects list` and no projects are returned
...
- **THEN** the skill runs `emergent projects create --name "<name>"` with a name derived from the repository directory or confirmed with the user
