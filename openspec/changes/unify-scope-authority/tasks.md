...
- [ ] 4.3 Add a single app-side **decision** check behind one seam — `superadmin_full` only, **no project tier, no org_admin** (requiring `superadmin_full`, so a `superadmin_readonly` row is refused; an `org_admin` membership is refused — issue #949, superseding the earlier any-org `org_admin` eligibility recorded here) — and route `apitoken.CanGrantAdminAll` through it instead of its bespoke `EXISTS` query; verify the existing `admin:all` minting tests still pass plus a `superadmin_full` allowed case, an `org_admin` refused case, a bare `project_admin` refused case, a `superadmin_readonly` refused case, and a neither-entitlement denied case
