...
- **Availability-driven provider picker:** the agent Sandbox settings page renders its provider `<select>` from the providers endpoint. `Auto` stays the default; unavailable providers are shown as disabled options labelled as unavailable (except the provider currently stored, which stays selectable so an unchanged submit never drops it); each provider's availability and reason are listed; a stored provider is never dropped from the form when the endpoint does not report it; and a failed/empty provider list renders an explicit warning instead of an unmarked list.
