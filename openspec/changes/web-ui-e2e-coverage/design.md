...
A spec that creates an entity removes it in `afterEach`/`finally`, and where the created resource is enumerable, a guard test asserts no live instance remains — the pattern from `specs/settings/mcp-servers-create-ui.spec.ts`. Scratch entities are named with an `E2E` prefix so the guard can match them. "Removed" is not always "deleted": graph objects and relationships are soft-deleted, so their rows stay in the archive (restorable via `POST /api/graph/objects/:id/restore` / `POST /api/graph/relationships/:id/restore`) and the guard can only assert absence from live listings and search results. Where the gateway exposes no delete route (graph objects), cleanup goes through the memory API with the signed-in user's own session token.
...
1. **`POST /blueprints/migrate/rollback` restores nothing — confirmed defect.**
   ...
   **Tracked as GitHub issue #513.** Until it is fixed, phase 3 proves migration **apply** (the undeclared property is dropped and archived) and the rollback route's 303/`migrateMsg` response contract — it does **not** prove rollback data restoration.
...
3. **The gateway exposes no object-delete route.** Object specs cannot clean up through the UI; they authenticate to the memory API with the signed-in user's own session token (from the `memory_session` cookie) plus `X-Project-ID`. Graph deletes are soft: `DELETE /api/graph/objects/:id` and `/api/graph/relationships/:id` retain the row, and both are restorable via `POST /api/graph/objects/:id/restore` / `POST /api/graph/relationships/:id/restore`. Cleanup therefore removes the entity from live listings/search, not from the database. Relevant to every future object spec.
