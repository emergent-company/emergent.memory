...
- **Optional builtin uniqueness index (defense-in-depth).** Migration 00157 adds a partial unique index `(name, version) WHERE source = 'builtin'` on `kb.graph_schemas`, deduping existing builtin duplicates first, so builtin schema identity is unambiguous going forward.
...
- `apps/server/migrations/00157_builtin_graph_schemas_unique.sql`: dedupe + partial unique index.
