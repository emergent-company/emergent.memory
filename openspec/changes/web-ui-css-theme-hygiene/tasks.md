...
- [ ] 6.8 Eyeball the sites whose radius necessarily moved, because daisyUI has no token for their old values: the 7 `rounded-md` sites (0.375 rem → `--radius-field`, 0.25 rem) and the `ui.templ` icon tile (`rounded-2xl` 1.0 rem → `--radius-box`, 0.5 rem), plus any go-daisy component using `rounded-xl`/`rounded-2xl` — notably `ui/stat-card.templ`, `ui/empty-state.templ`, `nav/page-title.templ`, `form/prompt-bar.templ`. Everything else is unchanged (20 `rounded-lg` sites stay 0.5 rem; 6 bare `rounded` stay 0.25 rem). This is the visual-regression checklist for §2; unit tests cannot see it.
