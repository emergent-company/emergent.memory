#!/usr/bin/env python3
"""
Domain-aware extraction e2e test — built-in /remember endpoint variant.

Tests that the POST /api/projects/:projectId/remember endpoint:
1. Ingests raw text, creates a document, and triggers the domain-remember-agent
2. New-domain documents trigger finalize-discovery (tool policy pauses for human approval)
3. User approves → schema pack created with a non-empty pack_name chosen by agent
4. Subsequent same-domain documents match existing packs via heuristic classifier
5. Re-extraction is queued after pack creation

Compared to run_domain_test.py (custom agent test), this test:
- Does NOT install a blueprint or create a runtime agent
- Uses POST /api/projects/:projectId/remember instead of ACP agent runs
- Passes file content as the "message" body (no pre-upload step)
- Polls runs via GET /api/v1/runs/:runId (global run lookup, no agent name needed)

Usage:
    EMERGENT_MEMORY_TOKEN=emt_... python3 run_domain_test_builtin.py
    EMERGENT_MEMORY_TOKEN=emt_... python3 run_domain_test_builtin.py --cleanup
    EMERGENT_MEMORY_TOKEN=emt_... python3 run_domain_test_builtin.py --project-id <existing-id>
    EMERGENT_MEMORY_TOKEN=emt_... python3 run_domain_test_builtin.py --doc 1
    EMERGENT_MEMORY_TOKEN=emt_... python3 run_domain_test_builtin.py --manual
"""
...
# ---------------------------------------------------------------------------
# Test documents: order matters — 1-5 establish packs, 6-7 match them
# ---------------------------------------------------------------------------
...
def create_project():
    ...
    r = requests.post(
        ...
        json={
            "name": f"Personal Assistant KB [builtin-test {int(time.time())}]",
            "orgId": ORG_ID,
        },
    )
    r.raise_for_status()
...
# ---------------------------------------------------------------------------
# /remember trigger + polling
# ---------------------------------------------------------------------------

def start_remember_run(project_id, filepath: Path):
    """Call POST /api/projects/:pid/remember with file content as message.
    Returns (run_id, document_id) from the 202 async response."""
    print(f"  Calling /remember for {filepath.name}...")
    content = filepath.read_text(encoding="utf-8", errors="replace")
    resp = post(f"/api/projects/{project_id}/remember", {
        "message": content,
        "schema_policy": "ask",
        "mode": "async",
    }, project_id=project_id)
    run_id = resp.get("run_id") or resp.get("runId") or ""
    document_id = resp.get("document_id") or resp.get("documentId") or ""
    print(f"  Run ID: {run_id}  Document ID: {document_id}")
...
def poll_remember_run(run_id, timeout=120):
    ...
    while time.time() < deadline:
        try:
            resp = get(f"/api/v1/runs/{run_id}")
            # Response is wrapped: {data: {...}} or unwrapped
...
def run_remember_with_auto_approve(project_id, filepath: Path):
    ...
    while True:
        ...
        if status == "input-required":
            ...
            print(f"  Run FAILED: {resp.get('error_message', resp.get('errorMessage', 'unknown error'))}")
            break
        elif status == "timeout":
...
def assert_remember_response(remember_resp):
    """Assert the /remember 202 response has expected shape."""
    results = []
    run_id = remember_resp.get("run_id") or remember_resp.get("runId") or ""
    doc_id = remember_resp.get("document_id") or remember_resp.get("documentId") or ""
    status = remember_resp.get("status") or ""
    results.append(check("remember returns run_id", bool(run_id), f"got={run_id!r}"))
    results.append(check("remember returns document_id", bool(doc_id), f"got={doc_id!r}"))
    results.append(check("remember status=running", status == "running", f"got={status!r}"))
    return results


def snapshot_document_stage(doc_id):
    """Fetch doc domain label/confidence — call before asserting post-agent state."""
    try:
        ...
        stage = signals.get("stage") or doc.get("domain_label") or (
            "new_domain" if not doc.get("domainName") else "classified"
        )
        return {
            "stage": stage,
...
def assert_agent_proposed_domain(project_id, run_id, tool_confirms):
    """Assert finalize-discovery was gated by tool policy confirm."""
    results = []
...
def assert_reextraction_queued(project_id, doc_id):
    """Assert reextraction job was queued, poll until complete."""
    results = []
    try:
        reextract_job = None
        for _ in range(12):
        ...
        results.append(check("reextraction job queued", reextract_job is not None, "count=0"))
...
MANUAL_RESPOND = False


# ---------------------------------------------------------------------------
# Schema count tracker
# ---------------------------------------------------------------------------
...
def run_test_doc(project_id, doc_config, idx, schema_count_before):
    ...
    # 1. Assert /remember response shape
    all_results += assert_remember_response(remember_resp)

    # 2. Assert document classification (same as custom agent test)
    if document_id:
        all_results += assert_document_classified(project_id, document_id, doc_config["expected_stage"], pre_agent_snapshot)
    ...
    if doc_config["expected_stage"] == "new_domain":
        ...
        if document_id:
            all_results += assert_reextraction_queued(project_id, document_id)
    else:
...
TEST_PROJECT_NAME_PREFIX = "Personal Assistant KB [builtin-test"
...
def cleanup_project(project_id):
    ...
    try:
        ...
        if TEST_PROJECT_NAME_PREFIX not in name:
            ...
            return
        delete_token = ORG_TOKEN or TOKEN
        requests.delete(
            f"{SERVER}/api/projects/{project_id}",
...
def main():
    parser = argparse.ArgumentParser(description="Domain-aware extraction e2e test — built-in /remember endpoint")
    parser.add_argument("--project-id", help="Reuse existing project instead of creating new")
    parser.add_argument("--keep-project", action="store_true", help="Do NOT delete project after test")
    parser.add_argument("--reset", action="store_true", help="Reset project state before running; requires --project-id")
    parser.add_argument("--doc", type=int, help="Run only this doc index (1-7)")
    ...
    print("Domain-Aware Extraction E2E Test — Built-in /remember endpoint")
    ...
    if args.project_id:
        set_project_id(project_id)
        # When reusing a project we don't have a project token yet — fall back to org/user token
