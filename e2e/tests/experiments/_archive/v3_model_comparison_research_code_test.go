...
func TestResearchAndCodeModelComparison_Flash25(t *testing.T) {
	runResearchAndCodeModelComparison(t, "deepseek-v4-flash")
}
...
func runResearchAndCodeModelComparison(t *testing.T, model string) {
	...
	setupTestProvider(t, rl, home, projectID, model)
	...
	rl.CLIStep("Trigger python-senior-coder-native",
		fmt.Sprintf("POST %s", triggerURL),
		fmt.Sprintf("HTTP %d: %s", triggerResp.StatusCode, truncateStr(triggerRespBody, 500)))
	...
	for time.Now().Before(deadline) {
		...
		if strings.Contains(compact, "status=success") || strings.Contains(compact, "status=completed") {
	...
	// ── Results ───────────────────────────────────────────────────────────────
	rl.Section("Verify WorkPackage final state and TaskResult output")
	...
	rl.CLI("memory graph objects list --type WorkPackage --project "+projectID, wpFinalOut)
	...
	rl.CLI("memory graph objects list --type TaskResult --project "+projectID, resultsOut)
