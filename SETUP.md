...
This starts the Docker dependencies (Postgres only - Zitadel is separate) with the workspace CLI process manager including health checks and log capture. To inspect their status or tail logs:
...
The workspace CLI handles preflight checks, dependency verification, and PID-based process management. Useful follow-up commands:
