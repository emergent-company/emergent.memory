...
- ✅ Running on port **5439** (separate from dev DB on 5437/5438)
...
docker stop e2e-db-1 && docker rm e2e-db-1
```
...
- name: Wait for Database
  run: |
    timeout 30 sh -c 'until docker inspect e2e-db-1 --format="{{.State.Health.Status}}" | grep -q healthy; do sleep 1; done'

- name: Run E2E Tests
  run: npx nx run server-go:test-e2e
