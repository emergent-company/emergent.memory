...
This guide documents the Phase 3 command surface for launching application services via the unified Nx + PID-based process orchestration pipeline.
...
# Start under process supervision
...
```bash
nx run server-go:setup --profile development
nx run server-go:start --profile development
nx run server-go:test -- --runInBand
```
...
- Process metadata originates from `tools/workspace-cli/src/config/application-processes.ts`.
- The CLI uses PID files stored in `logs/` for process tracking and lifecycle management.
...
| Symptom               | Resolution                                                                                                 |
| --------------------- | ---------------------------------------------------------------------------------------------------------- |
| `Conflicting process` | Run `npm run workspace:stop` or stop the foreign process before retrying.                                  |
| Missing dependencies  | Re-run `nx run <service>:setup` to ensure `npm install` completed.                                         |
| Port already in use   | Use `npm run workspace:status` to inspect running processes or `lsof -ti:<port>` to clear stale processes. |
