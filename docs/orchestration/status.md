...
application and dependency. The command surfaces process information alongside Docker Compose
...
| `uptimeSec`       | Seconds since process reported online.                                       |
| `restartCount`    | Process restart counter.                                                     |
| `ports`           | Accessible host ports advertised by the service or dependency.               |
| `detail`          | Summary of process state plus Docker status for dependencies.                |
  ...
  "services": [
    {
      ...
      "uptimeSec": 930,
      "restartCount": 0,
      "dependencyState": [{ "dependencyId": "postgres", "status": "online" }]
    },
    {
      "serviceId": "postgres",
      "type": "dependency",
...
| `process=missing`           | The dependency was not started. Launch via `nx run workspace-cli:workspace:deps:start`.                                                   |
