...
The workspace CLI manages processes via a PID-based system and Docker dependencies under a single command
...
3. Relaunch the managed service:
   ...
   ```
4. Re-run the status command to confirm ports are owned by the expected workspace processes.

### 4. CLI refuses to start due to stale state

**Symptoms**

- Commands return `Process already exists` or similar errors.

**Steps**

1. Flush lingering state by stopping all workspace processes:
   ```bash
