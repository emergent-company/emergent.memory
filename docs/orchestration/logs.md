...
```bash
# View only the Go API and its dependencies
...
All managed services write to directories under `apps/logs/`:
...
The workspace CLI includes log rotation configuration in `tools/workspace-cli/src/process/logrotate.ts`.
