# gVisor Runtime Setup for Agent Sandboxes

gVisor provides sandbox-level isolation for agent sandbox containers using the `runsc` OCI runtime. When gVisor is not available, the system falls back to the standard Docker runtime with a security warning.
...
When `ENABLE_AGENT_SANDBOXES=true`:

1. On startup, the sandbox provider checks if the `runsc` runtime is registered with Docker
2. If available, all agent sandbox containers are created with `--runtime=runsc`
...
| ---------------------------- | --------- | ------------------------------------------------- |
| `ENABLE_AGENT_SANDBOXES`     | `false`   | Enable sandbox endpoints and lifecycle            |
| `WORKSPACE_DEFAULT_PROVIDER` | `gvisor`  | Default sandbox provider                          |
| `WORKSPACE_DEFAULT_CPU`      | `2`       | Default CPU limit per sandbox                     |
| `WORKSPACE_DEFAULT_MEMORY`   | `4G`      | Default memory limit per sandbox                  |
| `WORKSPACE_DEFAULT_DISK`     | `10G`     | Default disk limit per sandbox                    |
| `WORKSPACE_MAX_CONCURRENT`   | `10`      | Maximum concurrent active sandboxes               |
| `WORKSPACE_WARM_POOL_SIZE`   | `0`       | Pre-booted container pool size (0 = disabled)     |
| `WORKSPACE_DEFAULT_TTL_DAYS` | `30`      | Default TTL for ephemeral sandboxes               |
...
When `WORKSPACE_NETWORK_NAME` is set (e.g., `workspace_net`), sandbox containers are attached to a dedicated Docker network. The `docker-compose.dev.yml` defines a `workspace_net` bridge network with inter-container communication disabled (`enable_icc=false`), preventing sandbox containers from reaching infrastructure services directly.
