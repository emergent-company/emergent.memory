...
Changed line 42 of `deploy/self-hosted/Dockerfile.server-with-cli` to use an absolute destination path:
...
1. Run local Docker build: `docker build -f deploy/self-hosted/Dockerfile.server-with-cli -t test-cli-build .`
2. Commit: `git add deploy/self-hosted/Dockerfile.server-with-cli && git commit -m "fix(docker): use absolute path for SDK COPY destination in CLI builder"`
