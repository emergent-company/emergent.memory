...
The workspace CLI uses a PID-based process manager to supervise services. Hot reload works seamlessly with this approach:

**Implementation:** `tools/workspace-cli/src/process/manager.ts`

**Key characteristics:**

- PID files stored in `logs/` directory for process tracking
- Automatic restart if process crashes
- Hot reload runs **inside** the supervised process
- Process manager doesn't interfere with file watching
...
- PID-based process manager: Custom workspace CLI solution
