...
| `MEMORY_ACCOUNT_API_KEY` | `api_key` | API key or `emt_*` token |
| `MEMORY_PROJECT_API_KEY` | `project_token` | Project-scoped token (overrides `api_key`) |
| `MEMORY_ORG_ID` | `org_id` | Default organisation ID |
| `MEMORY_PROJECT_ID` | `project_id` | Default project ID |

If `MEMORY_PROJECT_API_KEY` is set it takes precedence over `MEMORY_ACCOUNT_API_KEY` as the credential.
