...
Migration files live at `apps/server/migrations/` and are embedded in the binary.
The migration CLI is at `apps/server/cmd/migrate/main.go`.
...
To run a specific version or use advanced commands, use the CLI directly from `apps/server/`:
...
```bash
cd apps/server && go run ./cmd/migrate -c create <name>
```
...
This produces `apps/server/migrations/<next_version>_<name>.sql`.
...
cd apps/server && go run ./cmd/migrate -c mark-applied -v 1
...
Ensure the file ends in `.sql` and is inside `apps/server/migrations/`.
...
2. Generate the migration file with `go run ./cmd/migrate -c create <name>` from `apps/server/`.
