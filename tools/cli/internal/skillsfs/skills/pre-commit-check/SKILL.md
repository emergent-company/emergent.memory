...
### If any `apps/server/**/*.go` files changed
...
task test
# run from apps/server/ — equivalent to: go test ./... -v -count=1
...
# 3. Lint
task lint
# run from apps/server/ — runs golangci-lint run ./...
# Only run if the change touches non-trivial logic (not just docs/comments/config)
```

### If any `apps/server/domain/*/handler.go` files changed (Swagger hook)
...
git diff --cached --name-only --diff-filter=ACMR \
  | grep '^apps/server/domain/.*/handler\.go$' \
  | xargs -I{} grep -l "^// @Router" {}
```

If any handler file is missing `// @Router`, add the annotation block before
committing. See `apps/server/domain/chunks/handler.go` for examples.

### If any `apps/server/pkg/sdk/**/*.go` files changed

```bash
# SDK tests (separate module)
go test ./...
# run from apps/server/pkg/sdk/
```
...
| `task lint` | `apps/server/` |
| `task build` | `apps/server/` |
| SDK `go test ./...` | `apps/server/pkg/sdk/` |
...
- SDK is a separate Go module — `go test ./...` from `apps/server/pkg/sdk/`
  is independent of `go test ./...` from `apps/server/`
