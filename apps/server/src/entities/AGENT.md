...
| `AgentProcessingLog` | kb.agent_processing_log | Reaction agent processing state    |

#### Agent Trigger Types

The `Agent` entity supports three trigger types via the `triggerType` column:

| Trigger Type | Description                                    |
| ------------ | ---------------------------------------------- |
| `schedule`   | Runs automatically on cron schedule (default)  |
| `manual`     | Only runs when manually triggered via admin UI |
| `reaction`   | Runs in response to graph object events        |

#### Reaction Agent Configuration

For agents with `triggerType: 'reaction'`, the following columns control behavior:

| Column           | Type                 | Description                                        |
| ---------------- | -------------------- | -------------------------------------------------- |
| `reactionConfig` | `ReactionConfig`     | Object types, events, concurrency settings         |
| `executionMode`  | `AgentExecutionMode` | How actions are performed (suggest/execute/hybrid) |
| `capabilities`   | `AgentCapabilities`  | What operations the agent can perform              |

**ReactionConfig interface:**

```typescript
interface ReactionConfig {
  objectTypes: string[]; // Types to react to (empty = all)
  events: ('created' | 'updated' | 'deleted')[];
  concurrencyStrategy: 'skip' | 'parallel';
  ignoreAgentTriggered: boolean; // Ignore events from any agent
  ignoreSelfTriggered: boolean; // Ignore events from this agent
}
```
...
interface AgentCapabilities {
  ...
  allowedObjectTypes?: string[] | null; // null = all types
}
```
...
export class AgentProcessingLog {
  ...
  graphObjectId: string; // FK to GraphObject
  objectVersion: number; // Version when triggered
...
- Concurrency control (skip vs parallel strategy)
- Tracking processing status and errors
- Detecting stuck jobs (5-minute timeout for recovery)
