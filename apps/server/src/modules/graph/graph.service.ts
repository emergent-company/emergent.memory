...
import { EventsService, EmitOptions } from '../events/events.service';
import { ActorContext } from '../events/events.types';
...
/**
 * Actor context for graph operations - tracks who initiated the change
 * Used by reaction agents for loop prevention
 */
export type GraphActorContext = ActorContext;
...
export class GraphService {
  ...
  async createObject(
    ...
    // Extract actor context from input (defaults to 'user' if not provided)
    const actor: GraphActorContext = (input as any).actor ?? {
      actorType: 'user',
    };
      ...
      async () => {
        ...
        try {
          ...
          const row = await client.query<GraphObjectRow>(
            `INSERT INTO kb.graph_objects(type, key, status, properties, labels, version, canonical_id, project_id, branch_id, change_summary, content_hash, fts, embedding_updated_at, schema_version, actor_type, actor_id)
                     VALUES ($1,$2,$10,$3,$4,1,gen_random_uuid(),$5,$6,$7,$8, ${ftsVectorSql}, NULL, $11, $12, $13)
                     RETURNING id, project_id, branch_id, canonical_id, supersedes_id, version, type, key, status, properties, labels, deleted_at, change_summary, content_hash, fts, created_at, schema_version, actor_type, actor_id`,
            [
              type,
              key ?? null,
              properties,
          ...
          // Emit graph object created event for reaction agents
          if (this.eventsService && project_id) {
            try {
              ...
              // Non-blocking: event emission should not fail the operation
              this.logger.warn(
                `Failed to emit graph_object.created event: ${err}`
              );
            }
          }
  ...
  async patchObject(
    id: string,
    patch: PatchGraphObjectDto & {
      branch_id?: string | null;
      actor?: GraphActorContext;
    },
    ctx?: GraphTenantContext
  ): Promise<GraphObjectDto> {
    // Extract actor context from patch (defaults to 'user' if not provided)
    const actor: GraphActorContext = patch.actor ?? { actorType: 'user' };
    ...
    return this.runWithRequestContext(ctx, undefined, undefined, async () => {
      ...
      try {
        ...
        const inserted = await client.query<GraphObjectRow>(
          `INSERT INTO kb.graph_objects(type, key, status, properties, labels, version, canonical_id, supersedes_id, project_id, branch_id, deleted_at, change_summary, content_hash, fts, embedding_updated_at, schema_version, actor_type, actor_id)
                     VALUES ($1,$2,$15,$3,$4,$5,$6,$7,$8,$9,NULL,$10,$11, ${ftsVectorSql}, NULL, $16, $17, $18)
                     RETURNING id, project_id, branch_id, canonical_id, supersedes_id, version, type, key, status, properties, labels, deleted_at, change_summary, content_hash, fts, created_at, schema_version, actor_type, actor_id`,
          [
            current.type,
            current.key,
        ...
        // Emit graph object updated event for reaction agents
        const projectId = (current as any).project_id;
        if (this.eventsService && projectId) {
          try {
            ...
            // Non-blocking: event emission should not fail the operation
            this.logger.warn(
              `Failed to emit graph_object.updated event: ${err}`
            );
          }
        }
  ...
  async deleteObject(
    ...
    // Default actor context to 'user' if not provided
    const actorContext: GraphActorContext = actor ?? { actorType: 'user' };
    ...
    return this.runWithRequestContext(ctx, undefined, undefined, async () => {
      ...
      try {
        ...
        const tombstone = await client.query<GraphObjectRow>(
          `INSERT INTO kb.graph_objects(type, key, properties, labels, version, canonical_id, supersedes_id, project_id, deleted_at, fts, embedding_updated_at, actor_type, actor_id)
                     VALUES ($1,$2,$3,$4,$5,$6,$7,$8,CURRENT_TIMESTAMP, ${ftsVectorSql}, NULL, $12, $13)
                     RETURNING id, project_id, canonical_id, supersedes_id, version, type, key, properties, labels, deleted_at, fts, created_at, actor_type, actor_id`,
          [
            head.rows[0].type,
            head.rows[0].key,
        ...
        // Emit graph object deleted event for reaction agents
        const projectId = (head.rows[0] as any).project_id;
        if (this.eventsService && projectId) {
          try {
            this.eventsService.emitDeleted(
              'graph_object',
              tombstone.rows[0].id,
              projectId,
              {
                actor: actorContext,
                version: tombstone.rows[0].version,
                objectType: tombstone.rows[0].type,
              }
            );
          } catch (err) {
            // Non-blocking: event emission should not fail the operation
            this.logger.warn(
              `Failed to emit graph_object.deleted event: ${err}`
            );
          }
        }
