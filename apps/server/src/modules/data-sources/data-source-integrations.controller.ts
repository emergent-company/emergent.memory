...
export class DataSourceIntegrationsController {
  ...
  async createSyncConfiguration(
    ...
    const config = await this.service.createSyncConfiguration(
      id,
      ctx.projectId,
      dto,
      userId
    );
    // Reload scheduler if schedule is enabled
    if (config.schedule?.enabled) {
      await this.schedulerService.reloadConfig(id, config.id);
    }
    return config;
  }
  ...
  async updateSyncConfiguration(
    ...
    const config = await this.service.updateSyncConfiguration(
      id,
      ctx.projectId,
      configId,
      dto,
    ...
    await this.schedulerService.reloadConfig(id, configId);
    return config;
  }
  ...
  async deleteSyncConfiguration(
    ...
    await this.service.deleteSyncConfiguration(id, ctx.projectId, configId);
    // Unschedule the deleted configuration
    this.schedulerService.unscheduleConfig(id, configId);
  }
