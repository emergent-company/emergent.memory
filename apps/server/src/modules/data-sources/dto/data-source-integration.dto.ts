...
import { IsCronExpression } from '../../../common/decorators';
 ...
 * 1. Simple interval: `intervalMinutes` (15-1440 minutes)
 * 2. Cron expression: `cronSchedule` (e.g., "0 9 * * *" for daily at 9am)
 *
 * If both are provided, `cronSchedule` takes precedence.
 */
export class SyncScheduleDto {
  ...
  @ApiPropertyOptional({
    description:
      'Sync interval in minutes (15 min to 24 hours). Used if cronSchedule is not set.',
    example: 60,
    minimum: 15,
    maximum: 1440,
  })
  ...
  @ApiPropertyOptional({
    description:
      'Cron expression for scheduling (e.g., "0 9 * * *" for daily at 9am). Takes precedence over intervalMinutes.',
    example: '0 9 * * *',
  })
  @IsCronExpression({
    message:
      'Invalid cron expression. Expected format: "minute hour day-of-month month day-of-week" (e.g., "0 9 * * *" for daily at 9am)',
  })
  @IsOptional()
  cronSchedule?: string;
