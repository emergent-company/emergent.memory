...
export class AgentSchedulerService implements OnModuleInit, OnModuleDestroy {
  ...
  async scheduleAgent(agent: Agent): Promise<void> {
    const { id, name, strategyType, cronSchedule, triggerType } = agent;
    ...
    // Check if strategy exists for this strategyType
    if (!this.strategyRegistry.has(strategyType)) {
      this.logger.warn(
        `No strategy registered for agent strategyType "${strategyType}" (${name}). Skipping.`
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `Scheduled agent "${name}" (${strategyType}) with cron: ${cronSchedule}`
      );
    } catch (error) {
      this.logger.error(
  ...
  async executeAgent(agent: Agent): Promise<void> {
    const { id, name, strategyType } = agent;
    ...
    return this.tracer.startActiveSpan(
      ...
      async (span) => {
        ...
        span.setAttribute('agent.strategyType', strategyType);

        const strategy = this.strategyRegistry.get(strategyType);
        if (!strategy) {
          const availableStrategies = this.strategyRegistry.getRoles();
          const errorMessage = `No strategy registered for type "${strategyType}". Available strategies: [${availableStrategies.join(
            ', '
          )}]. Please update the agent's strategy type or implement a "${strategyType}" strategy.`;
          ...
          // Create a run record and mark it as failed so the user sees the error
          try {
            const failedRun = await this.agentService.startRun(id);
            await this.agentService.failRun(failedRun.id, errorMessage);
          } catch (runError) {
            this.logger.error(
              `Failed to create run record for missing strategy: ${
        ...
        // Use agent strategyType as trace type for filtering (e.g., 'merge-suggestion')
        const traceId = this.langfuseService?.createJobTrace(
          run.id,
          {
            name: `Agent: ${name}`,
            agentId: id,
            agentStrategyType: strategyType,
            agentName: name,
            cronSchedule: freshAgent.cronSchedule,
          },
          undefined, // environment (use default)
          `agent-${strategyType}` // traceType for filtering (e.g., 'agent-merge-suggestion')
        );
  ...
  /**
   * Execute an agent with reaction context
   * Used by ReactionDispatcherService for reaction-triggered agents
   *
   * @param agent The agent to execute
   * @param reactionContext The reaction context with event details
   */
  async executeAgentWithReaction(
    agent: Agent,
    reactionContext: import('./strategies').ReactionContext
  ): Promise<void> {
    const { id, name, strategyType } = agent;
    ...
    return this.tracer.startActiveSpan(
      'agent-scheduler.executeAgentWithReaction',
      async (span) => {
        span.setAttribute('agent.id', id);
        span.setAttribute('agent.name', name);
        span.setAttribute('agent.strategyType', strategyType);
        span.setAttribute(
          'reaction.graphObjectId',
          reactionContext.graphObjectId
        );
        span.setAttribute('reaction.eventType', reactionContext.eventType);
        span.setAttribute('reaction.objectType', reactionContext.objectType);
        ...
        const traceId = this.langfuseService?.createJobTrace(
          run.id,
          {
            name: `Agent: ${name} (reaction)`,
            agentId: id,
            agentStrategyType: strategyType,
            agentName: name,
            triggerType: 'reaction',
            reactionEventType: reactionContext.eventType,
            reactionObjectType: reactionContext.objectType,
            reactionObjectId: reactionContext.graphObjectId,
          },
          undefined,
          `agent-${strategyType}`
        );

        // Build execution context with reaction info
        const context: AgentExecutionContext = {
          agent: freshAgent,
          startedAt: run.startedAt,
          runId: run.id,
        ...
        try {
          ...
          if (strategy.shouldSkip) {
            const skipSpan = traceId
              ? this.langfuseService?.createSpan(traceId, 'shouldSkip', {
                  agentId: id,
                  reaction: {
                    objectId: reactionContext.graphObjectId,
                    eventType: reactionContext.eventType,
                  },
                }) ?? null
              : null;
          ...
          const executeSpan = traceId
            ? this.langfuseService?.createSpan(traceId, 'execute', {
                ...
                reaction: {
                  objectId: reactionContext.graphObjectId,
                  eventType: reactionContext.eventType,
                  objectType: reactionContext.objectType,
                },
              }) ?? null
            : null;

          this.logger.debug(
            `Executing agent "${name}" for ${reactionContext.eventType} event on ${reactionContext.objectType}...`
          );
          const result = await strategy.execute(context);
          ...
          if (result.skipReason) {
            ...
            await this.agentService.completeRun(run.id, result.summary);
            this.logger.log(
              `Agent "${name}" completed reaction for ${
                reactionContext.objectType
          ...
          await this.agentService.failRun(run.id, err.message);
          this.logger.error(
            `Agent "${name}" threw error during reaction: ${err.message}`,
            err.stack
          );
          span.setStatus({ code: SpanStatusCode.ERROR, message: err.message });
          span.recordException(err);
