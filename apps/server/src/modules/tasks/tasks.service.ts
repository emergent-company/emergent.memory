...
} from './suggestion-task-handler.service';
...
export interface ResolveResult {
  ...
  suggestionResult?: SuggestionApplyResult;
}
...
export class TasksService {
  ...
  constructor(
    ...
    @Inject(forwardRef(() => SuggestionTaskHandlerService))
    private readonly suggestionTaskHandler: SuggestionTaskHandlerService
  ) {}
   ...
   *
   * For suggestion tasks (object_create_suggestion, object_update_suggestion,
   * object_delete_suggestion, relationship_create_suggestion) that are accepted,
   * this will apply the suggested changes to the graph.
   */
  async resolve(
    taskId: string,
    userId: string,
    ...
    // For suggestion tasks that are accepted, apply the suggestion
    let suggestionResult: SuggestionApplyResult | undefined;
    if (
      this.suggestionTaskHandler.isSuggestionTask(task.type) &&
      status === 'accepted'
    ) {
      suggestionResult = await this.suggestionTaskHandler.applySuggestion(
        task,
        userId
      );

      if (!suggestionResult.success) {
        // If suggestion application failed, don't mark the task as resolved
        this.logger.error(
          `Suggestion application failed for task ${taskId}: ${suggestionResult.error}`
        );
        // Add error to notes but keep task pending for retry
        task.resolutionNotes = `Suggestion failed: ${suggestionResult.error}`;
        await this.taskRepo.save(task);
        return { task, suggestionResult };
      }

      // Add suggestion details to notes
      const summary = this.suggestionTaskHandler.getSuggestionSummary(task);
      notes = notes
    ...
    return { task: savedTask, mergeResult, suggestionResult };
  }
