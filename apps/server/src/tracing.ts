...
// Check if OpenTelemetry is enabled BEFORE importing heavy OTEL packages
...
// NOTE: All OpenTelemetry SDK imports are done CONDITIONALLY inside the if(isEnabled) block below
// to avoid loading heavy dependencies (@opentelemetry/sdk-node, etc.) when tracing is disabled.
// This prevents startup hangs caused by these packages in some environments.

// Light-weight API imports that are safe to use regardless of OTEL_ENABLED
// These are needed for type exports even when OTEL is disabled
import {
  trace,
  context,
  propagation,
...
// Store SDK reference for exports (using any to avoid importing heavy types when disabled)
...
if (!isEnabled) {
  ...
  // CONDITIONAL IMPORTS: Only load heavy SDK packages when OTEL is enabled
  // This prevents startup hangs in environments where these packages have issues
