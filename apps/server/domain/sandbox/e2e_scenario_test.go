...
func TestE2E_SetupExecutor_EmptyCommands(t *testing.T) {
	...
	ws := &AgentSandbox{ID: "ws-1", Provider: ProviderGVisor, ProviderWorkspaceID: "c-1"}
...
func TestE2E_AutoProvisioner_WorkspaceConfigParsing(t *testing.T) {
	...
	t.Run("valid config", func(t *testing.T) {
		...
		cfg, err := ParseAgentSandboxConfig(configMap)
		require.NoError(t, err)
	...
	t.Run("disabled config", func(t *testing.T) {
		cfg, err := ParseAgentSandboxConfig(map[string]any{"enabled": false})
		require.NoError(t, err)
		require.NotNil(t, cfg)
