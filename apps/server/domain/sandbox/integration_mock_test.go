...
func TestIntegration_ToolOps_ExecViaOrchestrator(t *testing.T) {
	...
	// Create a container
	result, err := provider.Create(t.Context(), &CreateContainerRequest{ContainerType: ContainerTypeAgentSandbox})
	require.NoError(t, err)
...
func TestIntegration_ToolOps_ReadWriteViaOrchestrator(t *testing.T) {
	...
	provider, _, err := o.SelectProvider(ContainerTypeAgentSandbox, DeploymentSelfHosted, ProviderGVisor)
	require.NoError(t, err)
...
func TestIntegration_Lifecycle_CreateDestroyFlow(t *testing.T) {
	...
	// Select and create
	provider, pt, err := o.SelectProvider(ContainerTypeAgentSandbox, DeploymentSelfHosted, "")
	require.NoError(t, err)
...
func TestIntegration_Snapshot_NotSupported(t *testing.T) {
	...
	provider, _, err := o.SelectProvider(ContainerTypeAgentSandbox, DeploymentManaged, "")
	require.NoError(t, err)
