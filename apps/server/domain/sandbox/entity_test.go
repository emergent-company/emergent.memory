...
func TestToResponse(t *testing.T) {
	...
	assert.Equal(t, ContainerTypeAgentSandbox, resp.ContainerType)
...
func TestToResponseList(t *testing.T) {
	now := time.Now()
	workspaces := []*AgentSandbox{
		{ID: "ws-1", ContainerType: ContainerTypeAgentSandbox, Provider: ProviderGVisor, Status: StatusReady, CreatedAt: now, LastUsedAt: now},
...
func TestToResponseList_Empty(t *testing.T) {
	resp := ToResponseList([]*AgentSandbox{})
