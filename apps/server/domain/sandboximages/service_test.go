...
func TestSandboxImage_ToDTO(t *testing.T) {
...
func TestSandboxImage_ToDTO_NilOptionals(t *testing.T) {
	img := &SandboxImage{
		ID:        "img-2",
		Name:      "coder",
...
func TestCreateSandboxImageRequest_Validate(t *testing.T) {
	tests := []struct {
		name    string
		req     CreateSandboxImageRequest
		wantErr bool
	}{
		{
			name:    "valid name only",
			req:     CreateSandboxImageRequest{Name: "my-image"},
			wantErr: false,
		},
		{
			name:    "valid with docker ref",
			req:     CreateSandboxImageRequest{Name: "py-ml", DockerRef: "python:3.12-slim"},
			wantErr: false,
		},
		{
			name:    "empty name",
			req:     CreateSandboxImageRequest{Name: ""},
			wantErr: true,
		},
	}
