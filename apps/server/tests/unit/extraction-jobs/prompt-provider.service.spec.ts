...
describe('ExtractionPromptProvider', () => {
  ...
  describe('getEntityExtractorPrompt', () => {
    ...
    it('should return Langfuse prompt when available', async () => {
      const mockPrompt = {
        name: EXTRACTION_PROMPT_NAMES.ENTITY_EXTRACTOR_JSON,
        version: 3,
        labels: ['production'],
  ...
  describe('getRelationshipBuilderPrompt', () => {
    const documentChunks = ['John lives in New York City.'];
    const entities = [
      {
        temp_id: 'person_john',
        name: 'John',
    ...
    it('should return Langfuse prompt when available', async () => {
      const mockPrompt = {
        name: EXTRACTION_PROMPT_NAMES.RELATIONSHIP_BUILDER_JSON,
        version: 2,
