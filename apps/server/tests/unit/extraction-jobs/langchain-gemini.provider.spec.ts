...
describe('LangChainGeminiProvider', () => {
  ...
  describe('Tool-based extraction (with relationshipSchemas)', () => {
    ...
    it('should deduplicate entities across chunks', async () => {
      ...
      // Note: confidence-based deduplication is no longer tested since confidence
      // is calculated by cascade system, not extracted from LLM
      expect(result.entities).toHaveLength(3);

      // Find the User Service entity - should exist (deduplication by name)
    ...
    it('should deduplicate relationships across chunks', async () => {
      ...
      // Should deduplicate relationships by source/target/type (case-insensitive)
      // Note: confidence-based deduplication is no longer tested since confidence
      // is calculated by cascade system, not extracted from LLM
      expect(result.relationships).toHaveLength(1);
    });
