...
describe('DatabaseService extended behaviour', () => {
  ...
  beforeEach(() => {
    vi.useRealTimers();
    // Ensure SKIP_DB is not set for these tests - we're testing actual database service behavior
  ...
  it('getOrgIdFromProjectId returns cached org ID on subsequent calls', async () => {
    ...
    const mockQuery = vi
      .fn()
      .mockResolvedValue([{ organization_id: 'test-org-123' }]);
    vi.spyOn(DataSource.prototype as any, 'query').mockImplementation(
      mockQuery
    );
