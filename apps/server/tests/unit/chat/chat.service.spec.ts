...
class DbMock {
  ...
  async runWithTenantContext<T>(
    ...
    // Simply execute the function - in tests we don't need actual tenant context
    return fn();
  }
}
...
function build(overrides?: { db?: DbMock; embeddingsEnabled?: boolean }) {
  ...
  const svc = new ChatService(
    ...
    db as any,
    new (class {
      /* embeddings service only used for retrieveCitations */
...
describe('ChatService (unit)', () => {
  ...
  it('offline createConversationIfNeeded creates private conversation & listConversations returns it (shared empty)', async () => {
    ...
    const convId = await svc.createConversationIfNeeded(
      undefined,
    ...
    expect(
      conv && typeof conv === 'object' && 'messages' in (conv as any)
    ).toBe(true);
  });
  ...
  it('getConversation returns null for invalid UUID and not-found for missing row', async () => {
    ...
    const { svc } = build({ db });
    expect(
      await svc.getConversation('invalid-id', null, null, null)
    ).toBeNull();
  ...
  it('rename/delete conversation offline paths', async () => {
    const { svc, db } = build();
    db.online = false;
    const id = await svc.createConversationIfNeeded(
      undefined,
      'Msg 1',
      null,
      null,
    ...
    expect(
      await svc.renameConversation(id, 'New Title', null, null, null)
    ).toBe('ok');
    expect(await svc.deleteConversation(id, null, null, null)).toBe('ok');
    expect(await svc.deleteConversation(id, null, null, null)).toBe(
      'not-found'
    );
  });
  ...
  it('retrieveCitations returns [] when embeddings disabled and when embed fails', async () => {
    ...
    expect(
      await svc.retrieveCitations('hello world', 5, null, null, null)
    ).toEqual([]);
  });
  ...
  describe('createConversationIfNeeded / rename / delete (online)', () => {
    it('reuses existing conversation id when valid and owner matches (no insert)', async () => {
      ...
      db.push({
        rows: [{ id: existingId, is_private: true, owner_id: 'user-1' }],
        rowCount: 1,
      });
      const { svc } = build({ db });
      const returned = await svc.createConversationIfNeeded(
        existingId,
        'Follow up message',
        'user-1',
        null,
      ...
      expect(
        db.queries.some((q) => /INSERT INTO kb.chat_conversations/.test(q.sql))
      ).toBe(false);
    });
    ...
    it('throws forbidden when reusing private conversation owned by different user', async () => {
      ...
      db.push({
        rows: [
          { id: existingId, is_private: true, owner_user_id: 'owner-xyz' },
        ],
        rowCount: 1,
      });
      const { svc } = build({ db });
      await expect(
        svc.createConversationIfNeeded(
          existingId,
          'Message',
          'user-other',
          null,
    ...
    it('creates new conversation (transaction happy path) when existingId invalid', async () => {
      ...
      const { svc } = build({ db });
      const newId = await svc.createConversationIfNeeded(
        '123e4567-e89b-12d3-a456-426614174099',
        'This is a brand new conversation starting now',
        'u1',
        null,
        null,
        true
      ...
      const inserts = db.queries.filter((q) =>
        /INSERT INTO kb.chat_conversations/.test(q.sql)
      );
    ...
    it('renameConversation online: not-found, forbidden, ok', async () => {
      ...
      expect(
        await svc.renameConversation(convId, 'Title A', 'user-x', null, null)
      ).toBe('not-found');
      expect(
        await svc.renameConversation(convId, 'Title B', 'user-x', null, null)
      ).toBe('forbidden');
      expect(
        await svc.renameConversation(convId, 'Title C', 'user-x', null, null)
      ).toBe('ok');
    });
    ...
    it('deleteConversation online: not-found, forbidden, ok', async () => {
      ...
      expect(await svc.deleteConversation(convId, 'user-x', null, null)).toBe(
        'not-found'
      );
      expect(await svc.deleteConversation(convId, 'user-x', null, null)).toBe(
        'forbidden'
  ...
  describe('listConversations (online filtering)', () => {
    ...
    it('applies org and project filters to shared & private queries', async () => {
      ...
      const sharedSQL =
        db.queries.find((q) =>
          /FROM kb.chat_conversations WHERE is_private = false/.test(q.sql)
        )?.sql || '';
      expect(sharedSQL).toMatch(/project_id IS NOT DISTINCT FROM/);
      const privSQL =
        db.queries.find((q) =>
          /FROM kb.chat_conversations WHERE is_private = true/.test(q.sql)
        )?.sql || '';
  ...
  describe('retrieveCitations', () => {
    it('returns fused citations (success path) including similarity field', async () => {
      ...
      class EmbedMock {
        async embedQuery(msg: string) {
          return [msg.length, 1, 2];
        }
      }
      ...
      const svc = new ChatService(
        ...
        mockMessageRepo,
        db as any,
        new EmbedMock() as any,
        new ConfigMock(true) as any
      );
      const out = await svc.retrieveCitations(
        'hello world',
        5,
        null,
        null,
    ...
    it('returns [] when embedQuery throws (embed failure path)', async () => {
      ...
      const svc = new ChatService(
        ...
        mockMessageRepo,
        db as any,
        new EmbedFail() as any,
        new ConfigMock(true) as any
      );
      const res = await svc.retrieveCitations(
        'message text',
        3,
        null,
        null,
  ...
  describe('message persistence + hasConversation', () => {
    it('offline persistUserMessage and persistAssistantMessage append & update timestamps', async () => {
      ...
      await svc.persistAssistantMessage(convId, 'Assistant reply', [
        {
          documentId: 'd1',
          chunkId: 'c1',
          chunkIndex: 0,
      ...
      expect(conv.messages.map((m: any) => m.role)).toEqual([
        'user',
        'user',
        'assistant',
      ]);
      ...
      expect(
        await svc.hasConversation('00000000-0000-0000-0000-000000000000')
      ).toBe(false);
    });
    ...
    it('online persistUserMessage / persistAssistantMessage execute expected queries', async () => {
      ...
      const newId = await svc.createConversationIfNeeded(
        '123e4567-e89b-12d3-a456-4266141740dd',
        'Seed message',
        'user-a',
        null,
        null,
      ...
      const assistantCreateCall = mockMessageRepo.create.mock.calls.find(
      ...
      expect(
        await svc.hasConversation('123e4567-e89b-12d3-a456-426614174099')
      ).toBe(false);
      expect(await svc.hasConversation(convId)).toBe(true);
    });
  });
