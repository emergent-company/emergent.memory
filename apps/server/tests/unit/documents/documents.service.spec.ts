...
class FakeChunkerService {
  async chunkDocument(_content: string, _options?: any): Promise<string[]> {
    return ['chunk1', 'chunk2'];
  }
}

// Mock ChunkEmbeddingJobsService
class FakeChunkEmbeddingJobsService {
  async createJob(_documentId: string): Promise<void> {
    return;
  }
}
...
// Helper to create DocumentsService with all required dependencies
function createDocumentsService(
  ...
  const chunker = new FakeChunkerService();
  const chunkEmbeddingJobs = new FakeChunkEmbeddingJobsService();
...
describe('DocumentsService extended behaviour', () => {
  ...
  it('list builds WHERE with org, project, and cursor', async () => {
    ...
    const dataSource = new ScriptableDataSource([
      {
        // Count query: SELECT COUNT(*)::int as total FROM kb.documents
        // params: [cursor.createdAt, cursor.id] (limit is sliced off for count)
        match: 'as total FROM kb.documents',
      ...
      {
        // Main query: SELECT d.id, ... FROM kb.documents
        // params: [limit+1, cursor.createdAt, cursor.id]
        match: 'SELECT d.id',
        respond: (_text, params) => {
          // Project filtering is now done via RLS, not query params
          // Since we pass a cursor, we expect 3 params
