...
export class FakeGraphDb {
  ...
  async query(sql: string, params?: any[]): Promise<any> {
    ...
    // Transaction control statements (no-op for fake DB)
    if (
      /^BEGIN$/i.test(sql.trim()) ||
      /^COMMIT$/i.test(sql.trim()) ||
      /^ROLLBACK$/i.test(sql.trim())
    ) {
      return { rows: [], rowCount: 0 };
    }
    // Advisory lock (no-op for fake DB)
    ...
    // SQL: INSERT INTO kb.graph_objects(type, key, status, properties, labels, version, canonical_id, supersedes_id, project_id, branch_id, deleted_at, change_summary, content_hash, fts, embedding_updated_at, schema_version, actor_type, actor_id)
    ...
    // Note: fts column may be followed by embedding, embedding_updated_at, schema_version, actor_type, actor_id depending on schema version
    if (
      /INSERT INTO kb\.graph_objects\(type, key, (?:status, )?properties, labels, version, canonical_id, supersedes_id, project_id, branch_id, deleted_at, change_summary, content_hash(?:, fts(?:, embedding)?(?:, embedding_updated_at)?)?(?:, schema_version(?:, actor_type, actor_id)?)?\)/i.test(
        sql
      )
    ) {
