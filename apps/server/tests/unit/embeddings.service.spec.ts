...
vi.mock('@langchain/google-genai', () => {
  return {
    GoogleGenerativeAIEmbeddings: class MockEmbeddingsClient {
      public embedQuery = vi.fn(async (text: string) =>
        Array.from({ length: EMBEDDING_DIMENSION }, (_, i) => i)
      );
      public embedDocuments = vi.fn(async (texts: string[]) =>
        texts.map((_, idx) =>
          Array.from({ length: EMBEDDING_DIMENSION }, () => idx)
        )
      );
    },
  };
...
describe('EmbeddingsService (Legacy - use Vertex AI in production)', () => {
  ...
  beforeEach(() => {
    ...
    // Also delete Vertex AI vars to ensure we test the Generative AI path
  ...
  it('ensureClient throws when enabled but GOOGLE_API_KEY missing', async () => {
    ...
    await expect(svc.embedQuery('hi')).rejects.toThrow(
  ...
  it('lazy initializes client and caches for subsequent calls (embedQuery)', async () => {
    ...
    // Verify the embedding is a valid vector (array of numbers)
  ...
  it('propagates underlying client errors', async () => {
    ...
    (client.embedQuery as any) = vi.fn(async () => {
      throw new Error('underlying fail');
    });
