...
class ChunkEmbeddingJobsServiceMock {
  createJob = vi.fn(async () => undefined);
  queueEmbeddingJob = vi.fn(async () => undefined);
}
...
function build(
  ...
  overrides?: Partial<{
    ...
    chunkEmbeddingJobs: ChunkEmbeddingJobsServiceMock;
  ...
  const svc = new IngestionService(
    ...
    (overrides?.chunkEmbeddingJobs ??
      new ChunkEmbeddingJobsServiceMock()) as any,
...
describe('IngestionService.ingestText', () => {
  ...
  it('embeddings failure + embedding column missing fallback path', async () => {
    ...
    const origQuery = db.query.bind(db);
    let metadataFallbackAttempted = false;
    (db as any).query = async (sql: string, params?: any[]) => {
      // First INSERT with embedding+metadata throws 42703 (simulates metadata column missing)
      // Then the retry without metadata also throws 42703 (simulates embedding column missing)
      // This triggers the embeddingColumnMissing counter
      if (/INSERT INTO kb.chunks/.test(sql)) {
        // Check if this is the retry without metadata (embedding column only)
        const hasEmbedding = /embedding/.test(sql);
        const hasMetadata = /metadata/.test(sql);

        if (hasEmbedding && !hasMetadata && !metadataFallbackAttempted) {
          // This is the retry after metadata column error - simulate embedding column missing
          metadataFallbackAttempted = true;
          const err: any = new Error('no embedding column');
          err.code = '42703';
          throw err;
        } else if (hasEmbedding && hasMetadata && !metadataFallbackAttempted) {
          // First attempt with both columns - simulate metadata column missing
          const err: any = new Error('no metadata column');
          err.code = '42703';
          throw err;
        }
      }
  ...
  it('handles 42P10 missing unique constraint for chunk upsert (retries without ON CONFLICT)', async () => {
    ...
    (db as any).query = async (sql: string, params?: any[]) => {
      // Match chunk inserts with ON CONFLICT - pattern now includes metadata column
      if (
        /INSERT INTO kb.chunks\(document_id, chunk_index, text/.test(sql) &&
