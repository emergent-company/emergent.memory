...
describe('RefinementContextAssembler (unit)', () => {
  ...
  describe('assembleContext', () => {
    ...
    it('includes schema when available from template pack', async () => {
      ...
      // 3. Incoming relationships query
      db.query.mockResolvedValueOnce({ rows: [], rowCount: 0 });
      // 4. Schema query (new format with typeName, description, jsonSchema)
    ...
    it('handles schema with null jsonSchema', async () => {
      const { service, db, objectChunksService } = build();
      ...
      db.query.mockResolvedValueOnce({ rows: [], rowCount: 0 });
      db.query.mockResolvedValueOnce({ rows: [], rowCount: 0 });
      // Schema query returns row with null jsonSchema
      db.query.mockResolvedValueOnce({
        rows: [{ typeName: 'Person', description: null, jsonSchema: null }],
        rowCount: 1,
      });
      ...
      // Should still return a schema with empty properties when row exists but jsonSchema is null
      expect(result.schema).toMatchObject({
        type: 'Person',
        properties: {},
      });
  ...
  describe('fetchRelationshipsWithDetails behavior', () => {
    it('fetches both outgoing and incoming relationships', async () => {
      ...
      // Verify outgoing query uses src_id (renamed from source_id)
      const outgoingCall = db.query.mock.calls[1];
      expect(outgoingCall[0]).toContain('src_id = $1');
      expect(outgoingCall[0]).toContain('dst_id = o.id');

      // Verify incoming query uses dst_id (renamed from target_id)
      const incomingCall = db.query.mock.calls[2];
      expect(incomingCall[0]).toContain('dst_id = $1');
      expect(incomingCall[0]).toContain('src_id = o.id');
    });
