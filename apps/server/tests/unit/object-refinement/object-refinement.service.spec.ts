...
function createMockConversationRepo() {
  const mockQueryBuilder = {
    ...
    execute: vi.fn().mockResolvedValue({ identifiers: [], generatedMaps: [] }),
  };
...
function createMockGraphService() {
  return {
    getObject: vi.fn().mockResolvedValue({
      id: 'obj-123',
      canonical_id: 'canonical-obj-123',
      type: 'Person',
      key: 'person-1',
      properties: { name: 'John Doe' },
      labels: [],
...
describe('ObjectRefinementService (unit)', () => {
  ...
  describe('getOrCreateConversation', () => {
    it('throws NotFoundException when object does not exist', async () => {
      const { service, graphService } = build();
      graphService.getObject.mockResolvedValue(null);
    ...
    it('creates a new conversation when none exists for the object', async () => {
      const { service, graphService, conversationRepo, messageRepo } = build();

      // Object exists (via graphService.getObject)
      graphService.getObject.mockResolvedValue({
        id: 'obj-123',
        canonical_id: 'canonical-obj-123',
        type: 'Person',
        properties: { name: 'John Doe' },
      ...
      // Should use createQueryBuilder for upsert
    ...
    it('returns existing conversation when one exists for the object', async () => {
      ...
      // Should NOT use createQueryBuilder since conversation exists
    ...
    it('uses object key as fallback when name is not set', async () => {
      ...
      graphService.getObject.mockResolvedValue({
        id: 'obj-456',
        canonical_id: 'canonical-obj-456',
        type: 'Document',
      ...
      conversationRepo.findOne
        ...
        .mockResolvedValueOnce({
          id: '123e4567-e89b-12d3-a456-426614174000',
          canonicalId: 'canonical-obj-456',
          objectId: 'obj-456',
          projectId: 'project-1',
          title: 'Refinement: doc-key-123 (Document)',
          createdAt: new Date(),
          updatedAt: new Date(),
        });
      messageRepo.count.mockResolvedValue(0);
    ...
    it('uses truncated object ID as fallback when neither name nor key is set', async () => {
      ...
      graphService.getObject.mockResolvedValue({
        ...
        canonical_id: 'canonical-12345678',
        type: 'Unknown',
        properties: {},
      });
      conversationRepo.findOne
        ...
        .mockResolvedValueOnce({
          id: '123e4567-e89b-12d3-a456-426614174000',
          canonicalId: 'canonical-12345678',
          objectId: '12345678-abcd-1234-abcd-123456789012',
          projectId: 'project-1',
          title: 'Refinement: 12345678 (Unknown)',
          createdAt: new Date(),
          updatedAt: new Date(),
        });
      messageRepo.count.mockResolvedValue(0);
  ...
  describe('applySuggestion', () => {
    ...
    it('returns error when object not found', async () => {
      const { service, messageRepo, graphService } = build();
      messageRepo.findOne.mockResolvedValue({
        id: 'msg-1',
        citations: {
          suggestions: [
    ...
    it('applies property_change suggestion successfully', async () => {
      ...
      // graphService.getObject called twice: once in applySuggestion, once in resolveHeadObjectId
      graphService.getObject.mockResolvedValue({
        id: 'obj-123',
        canonical_id: 'canonical-obj-123',
        version: 1,
    ...
    it('applies relationship_add suggestion successfully', async () => {
      const { service, messageRepo, graphService, db } = build();
