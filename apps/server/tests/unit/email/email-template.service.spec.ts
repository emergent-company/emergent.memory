...
describe('EmailTemplateService', () => {
  ...
  describe('render', () => {
    ...
    it('renders template with context', async () => {
      ...
      const result = await service.render('invitation', { name: 'Test User' });

      expect(result.html).toBe('<html><body>Test User</body></html>');
    });

    it('generates plain text from context fields', async () => {
      vi.mocked(fs.existsSync).mockReturnValue(true);
      ...
      const result = await service.render('invitation', {
        title: 'Welcome!',
    ...
    it('uses plainText from context if provided', async () => {
      vi.mocked(fs.existsSync).mockReturnValue(true);
      vi.mocked(fs.readFileSync).mockReturnValue('<mjml></mjml>');
    ...
    it('wraps content in layout when specified', async () => {
      vi.mocked(fs.existsSync).mockReturnValue(true);
      ...
      const result = await service.render('invitation', {}, 'default');

      expect(result.html).toContain('Layout with Template Content');
    });

    it('handles MJML validation warnings gracefully', async () => {
      vi.mocked(fs.existsSync).mockReturnValue(true);
      vi.mocked(fs.readFileSync).mockReturnValue('<mjml></mjml>');
