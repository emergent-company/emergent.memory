...
describe('EmailJobsService', () => {
  ...
  describe('enqueue', () => {
    it('creates a new job with pending status', async () => {
      ...
      // Should use dataSource.query with raw SQL INSERT
      expect(mockDataSource.query).toHaveBeenCalledWith(
        expect.stringContaining('INSERT INTO kb.email_jobs'),
        [
          'invitation',
          'user@example.com',
          'Test User',
    ...
    it('uses config maxRetries as default maxAttempts', async () => {
      ...
      // Check that maxAttempts (6th param) is 3 from config
      expect(mockDataSource.query).toHaveBeenCalledWith(
        expect.any(String),
        expect.arrayContaining([3]) // maxRetries from config
      );
    });
    ...
    it('allows custom maxAttempts to override config', async () => {
      ...
      // Check that maxAttempts (6th param) is 5, not 3
      const callArgs = (mockDataSource.query as ReturnType<typeof vi.fn>).mock
        .calls[0][1];
      expect(callArgs[5]).toBe(5); // 6th param is maxAttempts
    });
    ...
    it('sets null values for optional fields when not provided', async () => {
      ...
      const callArgs = (mockDataSource.query as ReturnType<typeof vi.fn>).mock
        .calls[0][1];
      expect(callArgs[2]).toBe(null); // toName
      expect(callArgs[4]).toBe('{}'); // templateData (JSON stringified empty object)
      expect(callArgs[6]).toBe(null); // sourceType
      expect(callArgs[7]).toBe(null); // sourceId
    });

    it('uses now() for nextRetryAt for immediate processing', async () => {
      const options = {
        templateName: 'test',
        toEmail: 'user@example.com',
        subject: 'Test',
      ...
      // The SQL should use now() for next_retry_at
      expect(mockDataSource.query).toHaveBeenCalledWith(
  ...
  describe('dequeue', () => {
    it('uses raw SQL with FOR UPDATE SKIP LOCKED', async () => {
      ...
      mockDataSource.query = vi.fn().mockResolvedValue(mockJobs);
