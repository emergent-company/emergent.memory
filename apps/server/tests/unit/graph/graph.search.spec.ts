...
describe('GraphService search', () => {
  test('object search filters by type and label with cursor pagination', async () => {
    const db = makeFakeGraphDb({
      enableSearch: true,
      enableRelationships: true,
    });
    const schemaRegistryStub = makeSchemaRegistryStub();
    const svc = new GraphService(db as any, schemaRegistryStub as any);
    const obj1 = await svc.createObject({
      type: 'Asset',
      key: 'key_1',
      properties: { a: 1 },
      labels: ['L1'],
    });
    const obj2 = await svc.createObject({
      type: 'Asset',
      key: 'key_2',
      properties: { a: 2 },
      labels: ['L1'],
    });
    const obj3 = await svc.createObject({
      type: 'Service',
      key: 'key_3',
      properties: { a: 3 },
      labels: ['L1'],
    });
    const first = await svc.searchObjects({
      type: 'Asset',
      label: 'L1',
      limit: 1,
    });
    ...
    const next = await svc.searchObjects({
      type: 'Asset',
      label: 'L1',
      limit: 1,
  ...
  test('relationship search filters by type/src/dst', async () => {
    ...
    await svc.createRelationship(
      { type: 'Rel', src_id: 'o_1', dst_id: 'o_2' },
      'org',
      'proj'
    );
    await svc.createRelationship(
      { type: 'Rel', src_id: 'o_2', dst_id: 'o_1' },
      'org',
      'proj'
    );
    const result = await svc.searchRelationships({
      type: 'Rel',
      src_id: 'o_1',
    });
  ...
  test('relationship search supports descending order newest-first pagination', async () => {
    ...
    const schemaRegistryStub = makeSchemaRegistryStub();
    const svc = new GraphService(db as any, schemaRegistryStub as any);
    // Create three objects so we can have three distinct relationships
    await svc.createObject({ type: 'O', key: 'ka', project_id: 'proj' } as any);
    await svc.createObject({ type: 'O', key: 'kb', project_id: 'proj' } as any);
    await svc.createObject({ type: 'O', key: 'kc', project_id: 'proj' } as any);
    // Insert three DISTINCT relationships in temporal order
    // Each has a unique (src_id, dst_id) pair to avoid version superseding
    await svc.createRelationship(
      { type: 'Link', src_id: 'o_1', dst_id: 'o_2', properties: { idx: 1 } },
      'org',
      'proj'
    );
    await svc.createRelationship(
      { type: 'Link', src_id: 'o_2', dst_id: 'o_3', properties: { idx: 2 } },
      'org',
      'proj'
    );
    await svc.createRelationship(
      { type: 'Link', src_id: 'o_1', dst_id: 'o_3', properties: { idx: 3 } },
      'org',
      'proj'
    );
    const firstPage = await svc.searchRelationships({
      type: 'Link',
      limit: 2,
      order: 'desc',
    });
    ...
    const secondPage = await svc.searchRelationships({
      type: 'Link',
      limit: 2,
      order: 'desc',
  ...
  test('relationship search supports ascending order oldest-first pagination', async () => {
    ...
    expect(firstPage.items[0].properties.idx).toBeLessThan(
      firstPage.items[1].properties.idx
    );
    expect(firstPage.next_cursor).toBeTruthy();
    if (firstPage.next_cursor) {
      ...
      if (secondPage.items.length) {
        expect(secondPage.items[0].properties.idx).toBeGreaterThan(
          firstPage.items[1].properties.idx
        );
      }
    }
