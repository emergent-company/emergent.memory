...
describe('GraphService - Type Registry Validation Integration', () => {
  ...
  describe('createObject - Type Registry Validation', () => {
    ...
    beforeEach(() => {
      ...
      // The createObject flow is now: BEGIN -> INSERT -> COMMIT (no advisory lock or key check)
      mockClient.query
    ...
    it('should skip Type Registry validation when project_id or org_id is missing', async () => {
      ...
      // Reset query plan - when org/project not provided, service queries GUCs first
      // Query order: GUC org -> GUC project -> BEGIN -> INSERT -> COMMIT
      mockClient.query.mockReset();
      mockClient.query
        .mockResolvedValueOnce({ rows: [{ org: null }] }) // GUC query for org_id
        .mockResolvedValueOnce({ rows: [{ proj: null }] }) // GUC query for project_id
        .mockResolvedValueOnce({ rowCount: 0 }) // BEGIN
        .mockResolvedValueOnce({
          rowCount: 1,
          rows: [
