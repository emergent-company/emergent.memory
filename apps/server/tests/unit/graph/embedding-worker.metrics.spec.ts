...
describe('EmbeddingWorkerService Metrics', () => {
  it('increments success and failure counters for processed batch', async () => {
    ...
    const jobs: Pick<
      ...
      'dequeue' | 'markCompleted' | 'markFailed'
    > = {
    ...
    const provider = {
      generate: vi.fn(async (text: string) => {
        ...
        // Return proper EmbeddingResult structure
        return {
    ...
    const worker = new EmbeddingWorkerService(
      jobs as any,
      db as any,
