...
class MockDatabaseClient {
  ...
  async query(sql: string, params: any[] = []): Promise<any> {
    ...
    // INSERT into merge_provenance (tracks merge history in separate table)
    if (normalizedSql.includes('insert into kb.merge_provenance')) {
      return { rows: [], rowCount: 2 }; // Two rows inserted (target and source roles)
    }
...
describe('ObjectMergeService', () => {
  ...
  describe('mergeObjects', () => {
    ...
    it('tracks merge provenance when enabled (default)', async () => {
      ...
      // Provenance is now tracked in kb.merge_provenance table, not in properties
      // Verify that provenance INSERT was executed
      const provenanceInserts = mockDb.client.queries.filter(
        (q) =>
          q.sql.toLowerCase().includes('insert into kb.merge_provenance') &&
          q.sql.toLowerCase().includes('target') &&
          q.sql.toLowerCase().includes('source')
      );
      expect(provenanceInserts.length).toBeGreaterThan(0);

      // Properties should NOT have _mergeHistory since it's stored separately
      const mergedProps = result.mergedProperties as Record<string, unknown>;
      expect(mergedProps._mergeHistory).toBeUndefined();
    });
    ...
    it('appends to existing merge history', async () => {
      // Set up an object with existing merge history in properties
      // (legacy data might still have _mergeHistory in properties)
      ...
      // The merged properties should preserve existing _mergeHistory from source
      // (since source-wins is default, and _mergeHistory is a property)
      const mergedProps = result.mergedProperties as Record<string, unknown>;

      // Verify provenance INSERT was executed for this merge
      const provenanceInserts = mockDb.client.queries.filter((q) =>
        q.sql.toLowerCase().includes('insert into kb.merge_provenance')
      );
      expect(provenanceInserts.length).toBeGreaterThan(0);

      // Existing _mergeHistory from source should be preserved in merged properties
      // but no new entry is added (provenance is tracked in separate table)
      expect(mergedProps._mergeHistory).toBeDefined();
      const historyArray = mergedProps._mergeHistory as Array<
        Record<string, unknown>
      >;
      expect(historyArray).toHaveLength(1); // Only the existing entry from source
  ...
  describe('edge cases', () => {
    ...
    it('handles both objects with empty properties', async () => {
      ...
      expect(result.success).toBe(true);
      // Merged properties should be empty object (provenance is tracked in separate table)
      expect(Object.keys(result.mergedProperties).length).toBe(0);

      // Verify provenance INSERT was executed
      const provenanceInserts = mockDb.client.queries.filter((q) =>
        q.sql.toLowerCase().includes('insert into kb.merge_provenance')
      );
      expect(provenanceInserts.length).toBeGreaterThan(0);
