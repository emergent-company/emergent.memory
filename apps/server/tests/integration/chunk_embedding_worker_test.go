...
func (s *ChunkEmbeddingWorkerTestSuite) createTestDocument() {
	_, err := s.testDB.DB.NewRaw(`
		...
		VALUES (?, ?, 'upload', 'test-document.txt', 'Test document content', now(), now())
...
func (s *ChunkEmbeddingWorkerTestSuite) TestWorker_StartStop() {
	...
	worker := extraction.NewChunkEmbeddingWorker(s.jobsService, mockEmbeds, s.testDB.DB, s.cfg, s.log, nil, nil, nil, false)
...
func (s *ChunkEmbeddingWorkerTestSuite) TestWorker_ProcessesHighPriorityFirst() {
	...
	worker := extraction.NewChunkEmbeddingWorker(jobsService, mockEmbeds, s.testDB.DB, smallBatchCfg, s.log, nil, nil, nil, false)
	err = worker.Start(s.ctx)
	s.NoError(err)
