...
func TestConstants(t *testing.T) {
	...
	if DefaultModel != "gemini-3-flash-preview" {
		t.Errorf("DefaultModel = %q, want gemini-3-flash-preview", DefaultModel)
	}
	...
	if DefaultTemperature != 0.0 {
		t.Errorf("DefaultTemperature = %v, want 0.0", DefaultTemperature)
	}
	if DefaultMaxOutputTokens != 65536 {
		t.Errorf("DefaultMaxOutputTokens = %d, want 65536", DefaultMaxOutputTokens)
	}
}
...
func TestConfig(t *testing.T) {
	t.Run("default values", func(t *testing.T) {
		...
		if cfg.MaxOutputTokens != 0 {
			t.Errorf("MaxOutputTokens should be 0 by default")
		}
	})
