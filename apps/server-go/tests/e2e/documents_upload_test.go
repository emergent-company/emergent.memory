...
func (s *DocumentsUploadTestSuite) TestUpload_RejectsWhenFileIsMissing() {
	// Skip this test in-process because storage check happens before file validation
	// In in-process mode without storage configured, we get 503 before the 400
	if s.testDB != nil {
		s.T().Skip("Skipping in-process: storage check happens before file validation")
	}
