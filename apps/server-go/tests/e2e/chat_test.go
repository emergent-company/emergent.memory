...
func (s *ChatTestSuite) TestProjectIsolation() {
	// Create a second project via API
...
func (s *ChatTestSuite) TestStreamChat_CreatesNewConversation() {
	...
	s.Equal(http.StatusOK, resp.StatusCode)
	s.Equal("text/event-stream", resp.Headers.Get("Content-Type"))
	s.Equal("no-cache", resp.Headers.Get("Cache-Control"))
...
func (s *ChatTestSuite) TestStreamChat_LLMNotConfigured() {
	// This test verifies behavior when LLM is not configured.
	// The test server has nil LLM client, so it should return a fallback message.
	// When running against an external server with LLM configured, this test may
	// get a real LLM response instead, which is also acceptable behavior.
	req := map[string]any{
		"message": "Hello",
	}
...
func (s *ChatTestSuite) TestStreamChat_ProjectIsolation() {
	// Create a second project via API
	project2ID := s.createProjectViaAPI("Second Project for Stream")
