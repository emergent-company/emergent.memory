 ...
 * @param projectId - Current project ID (required for recording activity)
...
export function createUserActivityClient(
  ...
  return {
    async recordActivity(payload: RecordActivityPayload) {
      try {
        if (!projectId) {
          console.debug('Cannot record activity: no project ID');
          return;
        }
        const url = `${apiBase}/api/user-activity/record?project_id=${projectId}`;
