...
const (
	...
	// minio/mc too), then withdrew the quay.io/minio images as well: anonymous
	// pulls now fail with "repository does not exist or may require 'docker
	// login'", which broke every fresh install and every `memory server
	// upgrade`. We now host our own mirror of the pinned images in the owned
	// ghcr.io/emergent-company registry.
	//
