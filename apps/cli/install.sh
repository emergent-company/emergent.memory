...
Channels:
  stable  Install the newest release via the /releases/latest pointer.
  dev     Install the rolling dev build from the 'cli-dev' tag.
EOF
}
...
main() {
    ...
    if [ "$channel" = "dev" ]; then
        version="cli-dev"
        log "Installing dev channel (tag cli-dev)"
    else
